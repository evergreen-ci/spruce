  ...
  constructor(props: PatchProps) {
    ...
    if (this.props.patch.description === "") {
      this.props.patch.description =
        "Patch from " +
        this.props.patch.author +
        " at " +
        datetime.format("LLLL") +
        " on project " +
        this.props.patch.project;
    }
    this.state = {
    ...
    const Variants = () => (
      <Grid container={true} spacing={2}>
        <Grid item={true} xs={12}>
          <Typography>
            Created at {this.state.datetime.format("LLLL")} on{" "}
            {this.state.project} [
            <Link href={this.state.reconfigureLink}>Reconfigure</Link>]
          </Typography>
        </Grid>
      ...
      <Grid>
        ...
        >
          <ExpansionPanelSummary expandIcon={<ExpandMoreIcon />}>
            <Typography className="patch-header">
