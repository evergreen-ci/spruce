...
export class Admin extends React.Component<Props, State> {
          ...
          <Card>
            <CardContent>
              <Grid container={true} spacing={2}>
                <Grid item={true} xs={6}>
                  <BannerCard
                    banner={this.state.config.banner}
                    onBannerTextChange={this.updateSingleConfigField("banner")}
                  />
                </Grid>
                <Grid item={true} xs={6}>
                  <Card>
                    <CardContent>Some other component here</CardContent>
                  </Card>
                </Grid>
              </Grid>
            </CardContent>
            <CardActions>
              <Button variant="outlined" className="save" onClick={this.save()}>
                Save
              </Button>
            </CardActions>
          </Card>
  ...
  private save = () => () => {
    this.props.APIClient.setAdminConfig({
      banner: this.state.config.banner
    } as models.AdminSettings).then((resp: AxiosResponse<any>) =>
