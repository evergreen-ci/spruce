...
export class LogContainer extends React.Component<Props, State> {
  ...
  public componentDidUpdate() {
    if (
      this.props.task.task_id !== this.state.taskId ||
      (this.state.isShowingTestLogs && !this.props.shouldShowTestLogs)
    ) {
      this.props.client
        .getLogs(
          this.props.task.task_id,
      ...
      this.props.shouldShowTestLogs &&
      this.state.rawLink !== this.props.test.logs.url_raw
    ) {
      if (this.props.test.logs.url_raw === "") {
        ...
        axios
          .get(this.props.test.logs.url_raw)
          .then((resp: AxiosResponse<any>) => {
            this.setState({
              logText: resp.data,
              logType: LogType.all,
    ...
    const Logs = () => (
      <InfiniteScroll loadMore={this.dummyLoadMore} className="log-scrollable">
        {this.state.logText.split("\n").map((textLine, index) => (
          <Typography
            className={
              this.lineContainsError(textLine.toLowerCase())
                ? "log-text-error"
                : "log-text-normal"
            }
            key={index}
    ...
    return (
      <Grid container={true} spacing={2} className="log-container">
        ...
        <Grid item={true} xs={8} className="log-button-group">
          ...
          >
            <ToggleButton className="log-button" key="all" value={LogType.all}>
              All Logs
            </ToggleButton>
            <ToggleButton
              ...
              key="task"
              value={LogType.task}
              disabled={this.state.isShowingTestLogs}
            >
              Task Logs
            </ToggleButton>
            <ToggleButton
  ...
  private handleLogTypeChange = (event: object, newLogType: LogType) => {
    this.props.client
      .getLogs(this.props.task.task_id, newLogType, this.props.task.execution)
      .then((resp: AxiosResponse<any>) => {
        this.setState(
          {
            logText: resp.data,
