...
export const stringifyMilliseconds = (
  input: string | number,
  skipDayMax: boolean,
  skipSecMax: boolean
) => {
  ...
  if (typeof input === "string") {
    ...
    if (input === 0) {
      ...
      return (
        Math.floor(input / MS_PER_MINUTE) +
        "m " +
        Math.floor((input % MS_PER_MINUTE) / MS_PER_SEC) +
        "s"
      );
    } else if (input < MS_PER_HOUR * 24 || skipDayMax) {
      return (
        Math.floor(input / MS_PER_HOUR) +
        "h " +
        Math.floor((input % MS_PER_HOUR) / MS_PER_MINUTE) +
        "m " +
        Math.floor((input % MS_PER_MINUTE) / MS_PER_SEC) +
        "s"
      );
...
export class TaskPanel extends React.Component<Props, State> {
  ...
  public componentDidMount() {
    this.props.client
      .getTestsForTask(this.props.task.task_id)
      ...
      <div>
        ...
        <Typography>
          ...
          {stringifyMilliseconds(
            this.props.task.expected_duration_ms,
            true,
            true
          ) + " on base commit"}
        </Typography>
        <Typography>
          ...
          <Link
            href={this.props.client.uiURL + "/host/" + this.props.task.host_id}
          >
        ...
        <Typography>
          ...
          {" Started on " +
            moment(this.props.task.start_time as Date).format("lll")}
        </Typography>
        <Typography>
          <Icon.DateRangeOutlined className="task-detail-icon" />
          {" Finished on " +
            moment(this.props.task.finish_time as Date).format("lll")}
        </Typography>
      </div>
    );
      ...
      <Grid container={true} direction="column" spacing={1}>
        {this.state.failingTests.length !== 0 ? (
          ...
          <Grid item={true} xs={12}>
            <Typography className="no-tests-text">No failing tests</Typography>
          </Grid>
        )}
      </Grid>
    );
      ...
      <Card>
        <CardActions>
          <Grid container={true} onClick={this.handleExpandClick} spacing={3}>
            <Grid item={true} xs={1}>
              {this.state.isShowingOtherTests ? (
                <Icon.ExpandLess />
              ) : (
                <Icon.ExpandMore />
              )}
        ...
        <Collapse
          in={this.state.isShowingOtherTests}
          timeout="auto"
          unmountOnExit={true}
        >
          <CardContent className="other-tests-list">
            {this.state.otherTests.length !== 0 ||
            this.state.skippedTests.length !== 0 ||
            this.state.silentFailTests.length !== 0 ? (
              ...
              <Typography className="no-tests-text">No other tests</Typography>
            )}
          </CardContent>
        </Collapse>
      </Card>
      ...
      return (
        <Grid item={true} xs={12} key={this.props.task.task_id}>
          <StyledExpansionPanel
            ...
            expanded={this.props.isCurrentTask}
          >
            <ExpansionPanelSummary
              expandIcon={<Icon.ExpandMore />}
              onClick={this.handleTaskClick}
            >
              <Typography className="task-detail-name">
                {this.props.task.display_name}
              </Typography>
              <div className="task-detail-status-parent">
                <Typography
                  className={
                    this.props.task.status + " task-detail-status-child"
                  }
                >
      ...
      return (
        <Grid item={true} xs={12} key={this.props.task.task_id}>
          ...
          >
            ...
            >
              <Typography
                color={this.props.status === "failed" ? "error" : "textPrimary"}
              >
                {this.props.task.display_name}
              </Typography>
            </ExpansionPanelSummary>
  ...
  private sortByTestFile(a: APITest, b: APITest) {
    const testFileA = a.test_file
      .split("/")
      .pop()
      .toUpperCase();
    const testFileB = b.test_file
      .split("/")
      .pop()
      .toUpperCase();
