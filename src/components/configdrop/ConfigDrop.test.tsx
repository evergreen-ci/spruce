...
describe("ConfigDrop", () => {
  ...
  const onInvalidLoadFinished = jest.fn(() => {
    ...
    expect(invalidWrapper.state("snackbarMessage")).toBe(
  ...
  const validWrapper = enzyme.shallow(
    <ConfigDrop
      updateClientConfig={verifyValidConfig}
      onLoadFinished={onValidLoadFinished}
    />
  );
  const invalidWrapper = enzyme.shallow(
    <ConfigDrop
      updateClientConfig={jest.fn()}
      onLoadFinished={onInvalidLoadFinished}
    />
  );
